...
func (rf *Raft) sendLog(peer int, logs []LogEntry) error {
	...
	if rf.sendAppendEntries(peer, request, response) {
		...
		return errors.New("sendAppendEntries rpc failed")
	}

	return nil
}

// 为每个 peer 创建一个协程用于复制日志
