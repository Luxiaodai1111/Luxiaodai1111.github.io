...
- I/O 请求范围跨过多个成员磁盘，这时需要把这个bio **分裂处理**，让每个裂片落在一个成员磁盘，就可以调用 generic_make_request 处理它。块 I/O 子系统提供了函数 bio_split 简化此项操作，并确保在所有裂片请求成功完成后，原 I/O 请求的完成回调函数才会被调用（向上层报告）。如果一切正常，本函数返回 0。这样块 I/O 子系统知道请求已处理完成，包括以成功状态完成和以错误状态完成。
